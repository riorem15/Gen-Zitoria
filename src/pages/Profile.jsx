...
import { Sun, Moon, LogOut, Save, User, MapPin, Globe } from 'lucide-react';
...
const AVATARS = [
  ...
  { id: 'owl', name: 'Burung Hantu', emoji: '🦉', color: 'from-brown-500 to-amber-700',
...
export default function Profile() {
  const { user, isDarkMode, toggleTheme, signOut, profileName, profileProvince, profileAvatar, profileCountry, setProfile, syncStats, points, bestStreak, unlockedBadges } = useStore();
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pt-2 pb-24 px-4 animate-in fade-in duration-500">
      ...
      <div className="glass-panel p-6 relative overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-3 mt-6">
          {[
            ...
            { label: 'Lencana', value: unlockedBadges.length },
          ].map(s => (
      ...
      <div className="glass-panel p-5">
        <h3 className="font-bold text-sm uppercase tracking-widest opacity-60 mb-4">Pilih Karakter</h3>
        <div className="grid grid-cols-3 gap-3">
          {AVATARS.map(av => (
            ...
            >
              <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${av.color} p-0.5 shadow-md`}>
                <div className="w-full h-full rounded-full overflow-hidden bg-white/10">
                  {av.svg}
                </div>
              </div>
      ...
      <div className="glass-panel p-5 space-y-4">
        <h3 className="font-bold text-sm uppercase tracking-widest opacity-60">Informasi Pribadi</h3>
        <div>
          <label className="text-xs font-bold opacity-60 flex items-center gap-1 mb-1"><User className="w-3 h-3"/>Nama</label>
        ...
        <div>
          <label className="text-xs font-bold opacity-60 flex items-center gap-1 mb-1"><MapPin className="w-3 h-3"/>Asal Provinsi</label>
          <input value={province} onChange={e => setProvince(e.target.value)}
            placeholder="Contoh: Jawa Barat, Banten..."
            className="w-full bg-surface/50 border border-glass-border rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:border-primary transition-colors" />
        </div>
        <div>
          <label className="text-xs font-bold opacity-60 flex items-center gap-1 mb-1"><Globe className="w-3 h-3"/>Negara</label>
      ...
      <div className="glass-panel p-5 space-y-3">
        <h3 className="font-bold text-sm uppercase tracking-widest opacity-60">Preferensi</h3>
        ...
        <button onClick={signOut}
          className="w-full flex items-center justify-center gap-2 p-4 rounded-xl text-error border-2 border-error/30 hover:bg-error/10 font-bold transition-all">
          <LogOut className="w-5 h-5" /> Keluar / Ganti Akun
        </button>
      </div>
    </div>
  );
